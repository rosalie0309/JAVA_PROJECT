...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        Scanner sc = new Scanner(System.in)
            ...
            for (int i = 0; i < nbBlocs; i++) {
                ...
                Future<byte[]> future = blocPool.submit(tache);
                resultats.add(future);
            }
            
            // Réassemblage tout en faisant le test sur la compétude des blocs reçus
    ...
    try {
        ...
        System.out.println("CLIENT_MAIN terminé : arrêt des téléchargements de blocs...");

        for (Future<byte[]> f : resultats) {
            f.cancel(true); // Interrompt les téléchargements
        }

        blocPool.shutdownNow(); // Stoppe tout proprement
